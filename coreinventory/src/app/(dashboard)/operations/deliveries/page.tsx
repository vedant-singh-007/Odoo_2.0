    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">Outgoing stock to customers</p>
        </div>
        <Link href="/operations/deliveries/new">
          <Button className="bg-[hsl(280,30%,35%)] hover:bg-[hsl(280,30%,30%)]" id="new-delivery-button">
      ...
      <Card className="border-0 shadow-md">
        <CardHeader className="pb-3">
          <CardTitle className="text-lg flex items-center gap-2">
            <ArrowUpFromLine className="h-5 w-5 text-[hsl(280,30%,35%)]" />
            Delivery Operations ({operations.length})
          </CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
            ...
            <TableBody>
                ...
                <TableRow key={op.id}>
                  ...
                  <TableCell>
                    <div className="space-y-1">
                      {op.moves.map((m) => (
                        <div key={m.id} className="text-sm">{m.product.name} × {m.quantity}</div>
                      ))}
                    </div>
                  </TableCell>
                  ...
                  <TableCell className="text-right">
                    {(op.status === "DRAFT" || op.status === "READY") && (
                      <div className="flex items-center justify-end gap-2">
                        <Button size="sm" onClick={() => handleValidate(op.id)} className="bg-emerald-600 hover:bg-emerald-700" id={`validate-${op.id}`}>Validate</Button>
                        <Button size="sm" variant="outline" onClick={() => handleCancel(op.id)} className="text-red-500 border-red-200 hover:bg-red-50">Cancel</Button>
                      </div>
                    )}
                  </TableCell>
                </TableRow>
              ...
              {operations.length === 0 && (
                <TableRow><TableCell colSpan={6} className="text-center text-gray-400 py-8">No deliveries found</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </CardContent>
